...
#line 138 "dla_lin_berr.f"
    safe1 = (*nz + 1) * safe1;
#line 140 "dla_lin_berr.f"
    i__1 = *nrhs;
#line 140 "dla_lin_berr.f"
    for (j = 1; j <= i__1; ++j) {
#line 141 "dla_lin_berr.f"
	berr[j] = 0.;
#line 142 "dla_lin_berr.f"
	i__2 = *n;
#line 142 "dla_lin_berr.f"
	for (i__ = 1; i__ <= i__2; ++i__) {
#line 143 "dla_lin_berr.f"
	    if (ayb[i__ + j * ayb_dim1] != 0.) {
#line 144 "dla_lin_berr.f"
		tmp = (safe1 + (d__1 = res[i__ + j * res_dim1], abs(d__1))) / 
			ayb[i__ + j * ayb_dim1];
/* Computing MAX */
#line 145 "dla_lin_berr.f"
		d__1 = berr[j];
#line 145 "dla_lin_berr.f"
		berr[j] = max(d__1,tmp);
#line 146 "dla_lin_berr.f"
	    }

/*     If AYB is exactly 0.0 (and if computed by SLA_yyAMV), then we know */
/*     the true residual also must be exactly 0.0. */

#line 151 "dla_lin_berr.f"
	}
#line 152 "dla_lin_berr.f"
    }
#line 153 "dla_lin_berr.f"
    return 0;
} /* dla_lin_berr__ */
